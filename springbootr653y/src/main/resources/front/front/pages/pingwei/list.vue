...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
			...
			<view class="cu-bar bg-white search" style="width:100%" :style="[{top:CustomBar + 'px'}]">
				<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
					...
					<view v-if="queryIndex==0" class="search-form round">
						...
						<input v-model="searchForm.pingweizhanghao" type="text" placeholder="评委账号" ></input>
					</view>
					<view v-if="queryIndex==1" class="search-form round">
						<text class="cuIcon-search"></text>
						<input v-model="searchForm.pingweixingming" type="text" placeholder="评委姓名" ></input>
					</view>
				<view class="action">
			...
			<view class="uni-product-list" :style='{"padding":"0px 24rpx 24rpx","margin":"60rpx 0 0","flexWrap":"wrap","background":"none","flex":"1","display":"flex","width":"calc(100% - 220rpx)","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"padding":"20rpx","margin":"0 0 40rpx","borderColor":"#86ce9f #e1f9eb","borderRadius":"60rpx","flexWrap":"wrap","borderWidth":"2rpx","background":"#fff","display":"flex","width":"48%","position":"relative","borderStyle":"solid","height":"auto"}' v-for="(product,index) in list" :key="index">
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto","order":"5"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('pingwei','修改')) || (!userid && isAuthFront('pingwei','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('pingwei','删除')) || (!userid && isAuthFront('pingwei','删除'))" @click.stop="onDeleteTap(product.id)">
		...
		<button :style='{"border":"0px solid #fff","boxShadow":"0 4rpx 8rpx rgba(0,0,0,.3)","color":"#fff","bottom":"120rpx","right":"128rpx","outline":"none","borderRadius":"100%","background":"#86CE9F","width":"88rpx","lineHeight":"88rpx","fontSize":"28rpx","position":"fixed","height":"88rpx","zIndex":"9999"}' v-if="userid && isAuth('pingwei','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"0px solid #fff","boxShadow":"0 4rpx 8rpx rgba(0,0,0,.3)","color":"#fff","bottom":"120rpx","right":"128rpx","outline":"none","borderRadius":"100%","background":"#86CE9F","width":"88rpx","lineHeight":"88rpx","fontSize":"28rpx","position":"fixed","height":"88rpx","zIndex":"9999"}' v-if="!userid && isAuthFront('pingwei','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
</mescroll-uni>
</template>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.pingweizhanghao="";
				this.searchForm.pingweixingming="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.pingweizhanghao){
					params['pingweizhanghao'] = '%' + this.searchForm.pingweizhanghao + '%'
				}
				if(this.searchForm.pingweixingming){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`pingwei`, params);
                } else {
                    res = await this.$api.list(`pingwei`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('pingwei', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.pingweizhanghao){
					searchForm['pingweizhanghao'] = '%' + this.searchForm.pingweizhanghao + '%'
				}
				if(this.searchForm.pingweixingming){
					searchForm['pingweixingming'] = '%' + this.searchForm.pingweixingming + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`pingwei`, searchForm);
                } else {
                    res = await this.$api.list(`pingwei`, searchForm);
                }
				// 如果是第一页数据置空
